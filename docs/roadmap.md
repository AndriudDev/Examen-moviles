...
- [x] Tests — jest + `jest-expo` instalados (`jest ~29.7.0` + `jest-expo ~57.0.5`, preset en `package.json`, `jest.setup.ts` con el mock oficial de AsyncStorage); **118 tests**. Lógica pura: `modelo/__tests__/validacion-test.ts` (RF-01: cada campo obligatorio con su mensaje, «no identificada» válido, límite 120 caracteres, cantidad entera ≥ 1), `modelo/__tests__/clima-test.ts` (tabla WMO completa con límites exactos y valores fuera de tabla → «Condición desconocida»), `modelo/__tests__/ClimaApi-test.ts` (`construirClima`, separada del fetch para probarse sin red), `vista/__tests__/formato-test.ts` (fechas legibles deterministas en cualquier zona horaria) y `vista/__tests__/tema-test.ts` (tokens de style.md con contraste WCAG AA ≥ 4.5:1). **Cobertura ≥ 70%** (`npm run test:coverage`): `consultarClima` completo en `modelo/__tests__/ClimaApi-consultar-test.ts` (caché con TTL, timeout con AbortController, reintento, HTTP no-OK, red caída, caché que falla sin bloquear), repositorio en `modelo/__tests__/RepositoryAvistamientos-test.ts` (AsyncStorage + expo-file-system mockeado: foto de caché → persistente, data URI web intacta, JSON corrupto), controladores en `controlador/__tests__/` (Registro: validación/normalización/fallo de persistencia; Listado: orden por fecha desc y filtro insensible a mayúsculas; Detalle: lugar guardado vs. reverse geocoding), adaptadores en `controlador/__tests__/` (`ubicacion-test.ts`: permisos, GPS con timeout 10 s, reverse geocoding con timeout 8 s y degradación; `camara-test.ts`: permisos y captura calidad 0.7) y fábricas de `modelo/Avistamiento.ts`. `npm test` → 118/118 con `coverageThreshold` global 70/70/70/70 en `package.json` (99.41% stmts, 100% lines); `npx tsc --noEmit` limpio.
...
- Fase 8 (Ingeniería): suite de **118 tests** con jest + `jest-expo` y **cobertura ≥ 70%** (`npm run test:coverage`, umbral 70/70/70/70 en `package.json`) sobre toda la capa de lógica — modelo (`validacion`, `clima`, `construirClima` y `consultarClima` de `ClimaApi`, `RepositoryAvistamientos` con expo-file-system mockeado, fábricas de `Avistamiento`), controladores (Registro: validación/normalización/persistencia; Listado: orden y filtro; Detalle: lugar guardado vs. geocoding) y adaptadores (`ubicacion`: permisos, GPS y geocoding con timeout; `camara`: permisos y captura 0.7) + tokens de `vista/tema.ts` con contraste WCAG AA → 99.41% stmts / 100% lines; `npx tsc --noEmit` limpio y `expo-doctor` 21/21; `package.json` con scripts `test`/`test:watch`/`test:coverage` y preset `jest-expo` (+ mock oficial de AsyncStorage en `jest.setup.ts`).